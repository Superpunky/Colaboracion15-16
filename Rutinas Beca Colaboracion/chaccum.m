...
if(isempty(parser))
    ...
    parser.addParamValue('ObjectPolarity','bright',@checkObjectPolarity);
    parser.addParamValue('EdgeThreshold',[],@checkEdgeThreshold);
end
...
validateRadiusRange(); % If Rmin and Rmax are the same then set R = Rmin.
    ...
    function tf = checkObjectPolarity(objectPolarity)
        validatestring(lower(objectPolarity), {'bright','dark'}, ...
